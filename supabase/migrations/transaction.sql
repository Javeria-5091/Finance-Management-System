-- ==========================================
-- TRANSACTION PAGE - RPC FUNCTIONS
-- ==========================================

-- Ensure required columns exist
DO $$ BEGIN
  ALTER TABLE finance.journal_entries ADD COLUMN IF NOT EXISTS reference TEXT DEFAULT '';
  ALTER TABLE finance.journal_entries ADD COLUMN IF NOT EXISTS entry_date DATE;
  ALTER TABLE finance.journal_entries ADD COLUMN IF NOT EXISTS project_id UUID;
  ALTER TABLE finance.journal_entries ADD COLUMN IF NOT EXISTS source_type TEXT DEFAULT 'JOURNAL';
  ALTER TABLE finance.journal_entries ADD COLUMN IF NOT EXISTS source_id UUID;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

-- 1. Summary KPIs
CREATE OR REPLACE FUNCTION reporting.transaction_summary()
...
-- 2. Transaction List
CREATE OR REPLACE FUNCTION reporting.transaction_list(
...
-- 3. Transaction Detail with Journal Lines + Settlement Waterfall
CREATE OR REPLACE FUNCTION reporting.transaction_detail(p_id UUID)
RETURNS JSON AS $$ DECLARE
  v_result JSON;

...
export default function CreditNotesPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const baseCurrency = organization?.base_currency || "PKR";
      ...
      if (currency !== baseCurrency) {
        ...
        if (!fxRate?.rate || Number(fxRate.rate) <= 0) {
          throw new Error(`No approved exchange rate found for ${currency}/${baseCurrency}. Add an exchange rate before creating this credit note.`);
        }
        exchangeRate = Number(fxRate.rate);
      }

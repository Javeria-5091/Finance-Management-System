...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    //
    // AR-02 FIX: public.clients has no client_code column (see schema.sql --
    // it was never added; same bug class already fixed in FND-BUD-03 for
    // src/app/api/clients/route.ts and src/app/api/projects/route.ts).
    // PostgREST rejects a select on an unknown column with PGRST204, which
    // made this endpoint return 500 on every single load once any receipt
    // existed. Select the real columns only (id, name).

...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: revision } = await supabase.from('budget_revisions').select('*').eq('id', body.data.revision_id).eq('budget_id', params.id).eq('organization_id', auth.orgId).maybeSingle();
  ...
  if (body.data.action === 'reject') {
    const { data, error } = await supabase.from('budget_revisions').update({ status:'REJECTED', approved_by:auth.userId, approved_at:new Date().toISOString() }).eq('id',revision.id).select().single();
    if(error) return NextResponse.json({error:error.message},{status:500});

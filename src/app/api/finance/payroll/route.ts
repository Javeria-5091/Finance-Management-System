...
export async function POST(req:NextRequest){const auth=await requirePermission('PAYROLL_UPDATE');
...
if(b.action==='post'){
    ...
    const {data:jid,error:jerr}=await supabase.schema('finance').rpc('post_journal_entry',{p_description:`Payroll ${r.payroll_period}`,p_transaction_date:r.period_end,p_period_id:b.period_id,p_lines:JSON.stringify([{account_id:b.salary_expense_account_id,debit_amount:r.total_gross_pay,credit_amount:0,description:'Payroll expense'},{account_id:b.payroll_payable_account_id,debit_amount:0,credit_amount:r.total_net_pay,description:'Payroll payable'}]),p_currency:'PKR',p_exchange_rate:1,p_source_type:'PAYROLL_RUN',p_source_id:r.id});
    if(jerr)
        throw jerr;

...
export async function POST(req:NextRequest)
{
    ...
    if(!b.invoice_id||!b.milestone_ref||!b.description||Number(b.amount)<=0||!b.milestone_date)
        return NextResponse.json({error:'invoice_id, milestone_ref, description, amount and milestone_date are required'},{status:400});
    const {data:inv}=await supabase.from('invoices').select('id').eq('id',b.invoice_id).eq('organization_id',auth.orgId).maybeSingle(); 
    if(!inv)

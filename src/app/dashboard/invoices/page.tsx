...
export default function InvoicesPage() {
  ...
  useEffect(() => { fetchInvoices(); fetchProjects(); }, [fetchInvoices, fetchProjects]);
    ...
    if (editingData) {
      ...
      if(user) await logAction(user.id, "Updated Invoice", "Invoice", `Updated ${data.invoice_number}`);
    } else {
      await supabase.from("invoices").insert({ ...data, user_id: user?.id });
      if(user) await logAction(user.id, "Created Invoice", "Invoice", `Created ${data.invoice_number} for ${data.client_name} of PKR ${data.amount}`);
    }
    ...
    if(user && inv) await logAction(user.id, "Deleted Invoice", "Invoice", `Deleted ${inv.invoice_number}`);
    setDeleteId(null);
    fetchInvoices();
  }

  // PDF GENERATION MAGIC FUNCTION - ENTERPRISE LEVEL
  function handleDownloadPDF(inv: Invoice) {
    ...
    // Left Side (Bill To)
    doc.setFont("helvetica", 'bold'); // FIXED HERE
    doc.text("BILL TO:", 14, y);
    doc.setFont("helvetica", "normal"); // FIXED HERE
    doc.setFontSize(12);

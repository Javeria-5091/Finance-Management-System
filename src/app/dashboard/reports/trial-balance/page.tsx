...
export default function TrialBalancePage() {
  ...
  const generateReport = async () => {
    if (!periodFrom || !periodTo) return alert("Please select 'From' and 'To' periods");
    const fromIndex = periods.findIndex(p => p.id === periodFrom);
    const toIndex = periods.findIndex(p => p.id === periodTo);
    if (fromIndex === -1 || toIndex === -1 || fromIndex > toIndex) return alert("Invalid period selection range");
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div>
            ...
            <select value={selectedFY} onChange={(e) => handleFYChange(e.target.value)} className="w-full px-3 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
          ...
          <div>
            ...
            <select value={periodFrom} onChange={(e) => setPeriodFrom(e.target.value)} disabled={!selectedFY} className="w-full px-3 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none disabled:opacity-50 disabled:cursor-not-allowed">
              <option value="">Select Start...</option>
              {periods.map(p => (
                <option key={p.id} value={p.id}>{p.name} ({new Date(p.start_date).toLocaleDateString('en-PK', { day: '2-digit', month: 'short' })})</option>
              ))}
            </select>
          </div>
          <div>
            ...
            <div className="flex gap-2">
              <select value={periodTo} onChange={(e) => setPeriodTo(e.target.value)} disabled={!selectedFY} className="flex-1 px-3 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none disabled:opacity-50 disabled:cursor-not-allowed">
                <option value="">Select End...</option>
                {periods.map(p => (
                  <option key={p.id} value={p.id}>{p.name} ({new Date(p.end_date).toLocaleDateString('en-PK', { day: '2-digit', month: 'short' })})</option>
                ))}
              </select>
              <button onClick={generateReport} disabled={!periodFrom || !periodTo || loading} className="px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors whitespace-nowrap">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
              {data.map(row => (
                <tr key={row.account_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                  ...
                  <td className="p-4 text-right text-gray-700 dark:text-gray-300">{Number(row.total_debit) > 0 ? Number(row.total_debit).toLocaleString('en-PK', { minimumFractionDigits: 2 }) : '-'}</td>
                  <td className="p-4 text-right text-gray-700 dark:text-gray-300">{Number(row.total_credit) > 0 ? Number(row.total_credit).toLocaleString('en-PK', { minimumFractionDigits: 2 }) : '-'}</td>
                  <td className="p-4 text-right font-semibold text-gray-900 dark:text-white">{Number(row.net_balance).toLocaleString('en-PK', { minimumFractionDigits: 2 })}</td>
                </tr>
              ))}
            </tbody>
            <tfoot className="bg-gray-50 dark:bg-gray-900/50 font-bold border-t-2 border-gray-200 dark:border-gray-600">
              <tr>
                ...
                <td className="p-4 text-right text-gray-900 dark:text-white">{totalDr.toLocaleString('en-PK', { minimumFractionDigits: 2 })}</td>
                <td className="p-4 text-right text-gray-900 dark:text-white">{totalCr.toLocaleString('en-PK', { minimumFractionDigits: 2 })}</td>
                <td className="p-4 text-right text-gray-900 dark:text-white">{(totalDr - totalCr).toLocaleString('en-PK', { minimumFractionDigits: 2 })}</td>
              </tr>
            </tfoot>
          </table>
        </div>
